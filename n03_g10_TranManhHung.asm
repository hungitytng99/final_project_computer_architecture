...
ReadKey:
	...
	nop
	beqz $t5,WaitForDis	# Start time first tiem after press first key
	li $v0, 30 		# Start time value in t8
	syscall
	addi $t8, $a0, 0
	la $a1, input		# a1 = input[0]
	la $a0, source 		# a0 = source[0]
...
WaitForDis:
	...
	nop
	beq $t2, $zero, WaitForDis # if $t2 == 0 then wait
	nop
StoreKey:
	add $s1, $a1, $s0 	# s1 = address of input[0] + i
	beq $t0, 10, DISPLAY_LED 	# incase the input key is ENTER
...
continue:
	...
	beq $s6,$s5,DISPLAY_LED 	# When input string length reaches source string length
	j input_loop
	nop
####################### Compare string and display LED section
DISPLAY_LED:
	addi $s0,$zero,0 	# i = 0
